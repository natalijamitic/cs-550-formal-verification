...
lemma aft_tree: "nodes(explode n t) = (nodes t + 1)*(2^n) - 1"
...
lemma xySwap: "incXby dx  O  incYby dy  =  incYby dy  O  incXby dx "
   by (auto simp add: incYby_def incXby_def)
...
lemma "assignX (\<lambda> s. x_in s + k) = incXby k" 
   by (auto simp add: incXby_def assignX_def ) 
...
lemma "Range {(x,y)|x y. P x y} = {y. \<exists> x. P x y}" 
   by (auto simp add: sp_def)
