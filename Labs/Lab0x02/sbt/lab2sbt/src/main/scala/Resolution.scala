...
object Resolution {
  ...
  def skoleHelp(f: Formula, memory_list: List[Term], memory_map: Map[Identifier, Term]): Formula = {
    f match {
      ...
      case Exists(variable, inner)    => {
        ...
        var new_memory_list = unique(freeIdentsList ++ memory_list)

        // add exists identifier to map with Function of previously collected list
        val new_memory_map = memory_map ++ Map(variable.name -> Function(variable.name, new_memory_list))
  ...
  def solveMansionMystery: Unit = {
    ...
    val prenex = conjunctionPrenexSkolemizationNegation(mansionMystery)

  



    val assumptions: List[(Clause, Justification)] = prenex.map { (_, Assumed) }
    
    val s = List[(Clause, Justification)](
      (
        List(Atom(hates(a, a))),
      ...
      (
        List(Atom(hates(b, a))),
        Deduced((13, 16), Map(Synthetic(9) -> a))
      ),
      (
        List(Atom(Neg(hates(c, a)))),
        Deduced((8, 16), Map(Synthetic(4) -> a))
      ),
      (
        List(Atom(Neg(killed(c, a)))),
        Deduced((6, 18), Map(Synthetic(2) -> c, Synthetic(3) -> a))
      )
    )
    println("Is the proof correct? : " + checkResolutionProof(assumptions ++ s))
  }
