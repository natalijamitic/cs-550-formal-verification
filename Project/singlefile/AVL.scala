...
object AVL {
    ...
    def delete_max(tree: Tree): (BigInt, Tree) = {
        ...
        tree match {
            case Node(n, l, r) if r.size == 0 => {
                ...
                check(l.height == tree.height -1)
                check(r.size == 0 && (l.size + 1) == tree.size)
                (n,l)
            }
            case Node(n, l, r) => {
    ...
    }.ensuring(res => res._2.isAVL && (res._2.size + 1 == tree.size) && (res._2.height == tree.height || res._2.height == tree.height - 1) && res._2.checkGreatest(res._1) && tree.getKeySet.contains(res._1) && res._2.getKeySet.subsetOf(tree.getKeySet))
    ...
    }.ensuring(res => res.isAVL && (res.size + 1 == tree.size) && res.getKeySet.subsetOf(tree.getKeySet) && (tree.height == res.height || tree.height == res.height + 1))
    ...
    }.ensuring(res => res.isAVL && ((res.size == tree.size) || (res.size + 1 == tree.size)) && res.getKeySet.subsetOf(tree.getKeySet) && (tree.height == res.height || tree.height == res.height + 1))
